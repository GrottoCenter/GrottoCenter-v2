...
?>
    ...
    function massifOnClick(e, id) {
      detailMarker(e, 'massif', id, '<?php echo $_SESSION['language']; ?>', false);
    }
<?php
	break;
	case "delete":
...
<?php
    $sql = "SELECT DISTINCT ";
    $sql .= "T_massif.Id AS `0`, ";
    $sql .= "IF((T_massif.Locked = 'YES' AND NOT T_massif.Id_locker = '".$_SESSION['user_id']."'),1,0) AS `1`, ";
    $sql .= "CONCAT_WS('',CONCAT_WS(' /!\\\\ ',T_massif.Name,IF(T_massif.Locked = 'YES','<convert>#label=42<convert>',NULL)),'[|]',IF((T_massif.Locked = 'YES' AND T_massif.Id_locker = '".$_SESSION['user_id']."'),'".str_replace("'","''","<convert>#label=573<convert>")."',NULL),'[|]',GROUP_CONCAT(DISTINCT T_country.".$_SESSION['language']."_name ORDER BY T_country.".$_SESSION['language']."_name SEPARATOR ', ')) AS `2`, ";//Nom du massif //Pays
    $sql .= "T_massif.Name AS `3`, ";//Nom du massif
    $sql .= "T_country.".$_SESSION['language']."_name AS `4`, ";//Pays
    $sql .= "IF((T_massif.Locked = 'YES' AND T_massif.Id_locker = '".$_SESSION['user_id']."'),1,0) AS `5` ";
    ...
    $columns_params = array(
			...
			2 => "[hidden]|<convert>#label=551<convert><br /><convert>#label=98<convert>",
			3 => "T_massif*Name|[hidden]<convert>#label=551<convert>",
