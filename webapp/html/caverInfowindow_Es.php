		...
		switch ($caver[0]['Contact_is_public']) {
			case 1: //Contact visible uniquement par les inscrits a GrottoCenter
				if (!USER_IS_CONNECTED) {
					$contact = "<a href='connection_".$_SESSION['language'].".php?type=login' target='filter'><i>Favor de conectarse.</i></a>";//Connectez vous
				}
				break;
			case 2: //Contact toujours visible
				break;
			default:
				$contact = "<i>Privado</i>";//Privé
				break;
		}
		//Number of grottoes
...
<?php if (!$clustered) { ?>
  	<div id="GC_IW_header_div" class="headercaver <?php if ($caver_is_referent) { echo "referent_"; } ?>caver<?php if ($caver_is_connected) { echo "_connected"; } ?>">
  		<table cellspacing="0" cellpadding="0" border="0" width="100%"><tr><td style="width:50%;text-align:right;line-height:32px;">Espeléologo<!--Spéléologue--></td>
    ...
    <div class="content">
        ...
        <a href="JavaScript:showRelationList('<?php echo $_SESSION['language']; ?>','entry','caver','<?php echo $id; ?>');" title="Mostrar la lista"><!--Voir la liste-->
          <span class="value_entry">
            <?php echo $entries[0]['Count']; ?>
          </span>
        </a> 
        <a href="JavaScript:showRelationList('<?php echo $_SESSION['language']; ?>','grotto','caver','<?php echo $id; ?>');" title="Mostrar la lista"><!--Voir la liste-->
          <span class="value_grotto">
            <?php echo $grottoes[0]['Count']; ?>
          </span>
        </a> 
        <a href="JavaScript:showRelationList('<?php echo $_SESSION['language']; ?>','contribution','caver','<?php echo $id; ?>');" title="Mostrar la lista"><!--Voir la liste-->
        ...
        <!--<span class="value" id="GC_IW_isConnected" style="display:none;float:right;">(está conectado)</span>--><!--Est connecté-->
    	</div>
    	<div class="label">
...
<?php if ($grottoes_list != "") { ?>
    	<div class="label">
        Clubs<!--Clubs--> : <span class="value"><?php echo $grottoes_list; ?></span>
    	</div>
<?php } ?>
...
<?php } ?>
        ...
        Direcciones<!--Itinéraire--> : <a href="JavaScript:getDirectionsForm(false);">Hasta aquí<!--Vers ce lieu--></a> - <a href="JavaScript:getDirectionsForm(true);">Desde aquí<!--Depuis ce lieu--></a>
      </div>
		</div>
		<div class="footer">
			<label for="link"><input type="checkbox" name="link" id="GC_IW_link" class="inputIW" title="Mostrar la relación entre los elementos" style="border: none;" onclick="Javascript:switchLines(<?php echo $id; ?>, 'caver', this.checked);" />Enlances<!--Liens--></label><!--Afficher les liens-->
			<span class="details">
<?php
      if ($caver[0]['Id_comment'] != "" && False) { //DESACTIVE !!!
?>
        <a href="comments_<?php echo $_SESSION['language']; ?>.php?caver_id=<?php echo $id; ?>" target="_blank" title="Ver los comentarios hechos por este espeléologo"><!--Voir les commentaires déposés par ce spéléologue-->
				  Comentarios…<!--Commentaires...-->
			  </a><br />
<?php
      }
?>
        <a href="#" onclick="JavaScript:detailMarker(event, 'caver', '<?php echo $id; ?>', '<?php echo $_SESSION['language']; ?>');"  title="Mostrar las propiedades de este elemento"><!--Obtenir les propriétés de ce marqueur-->
				  Propiedades<!--Propriétés-->
			  </a><br />
			  <a href="contact_<?php echo $_SESSION['language']; ?>.php?type=message&amp;subject=bad_content&amp;category=caver&amp;name=<?php echo $caver[0]['Login']; ?>" target="filter" title="Notificar abuso"><!--Signaler du contenu hors-charte-->
				  Notificar abuso<!--Hors-charte-->
        </a>
      </span>
		</div>
<?php
...
?>
  	<div class="header cavers_clustered" style="height:42px;">
  		Espeléologos<!--Speleologues-->
  	</div>
    <div class="content" style="height:170px;">
      <div class="info">
        <?php echo getTopBubble(); ?>
        Para detallar cada elemento, por favor, zoom.<!--Pour détailler chaque marqueur, veuillez zoomer.-->
...
<?php while(list($k,$v) = each($cavers)) { if ($v['Nickname'] != "") { ?>
            ...
            <a href="#" onclick="JavaScript:detailMarker(event, 'caver', '<?php echo $v['Id']; ?>', '<?php echo $_SESSION['language']; ?>');" title="Mostrar las propiedades de este elemento"><!--Obtenir les propriétés de ce marqueur-->
    				  Propiedades<!--Propriétés-->
    			  </a>
    			</td></tr>
<?php } } ?>
