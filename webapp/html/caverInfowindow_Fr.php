		...
		switch ($caver[0]['Contact_is_public']) {
			case 1: //Contact visible uniquement par les inscrits a GrottoCenter
				if (!USER_IS_CONNECTED) {
					$contact = "<a href='connection_".$_SESSION['language'].".php?type=login' target='filter'><i>Connectez vous.</i></a>";//Connectez vous
				}
				break;
			case 2: //Contact toujours visible
				break;
			default:
				$contact = "<i>Privé</i>";//Privé
				break;
		}
		//Number of grottoes
...
<?php if (!$clustered) { ?>
  	<div id="GC_IW_header_div" class="headercaver <?php if ($caver_is_referent) { echo "referent_"; } ?>caver<?php if ($caver_is_connected) { echo "_connected"; } ?>">
  		<table cellspacing="0" cellpadding="0" border="0" width="100%"><tr><td style="width:50%;text-align:right;line-height:32px;">Spéléologue<!--Spéléologue--></td>
    ...
    <div class="content">
        ...
        <a href="JavaScript:showRelationList('<?php echo $_SESSION['language']; ?>','entry','caver','<?php echo $id; ?>');" title="Voir la liste"><!--Voir la liste-->
          <span class="value_entry">
            <?php echo $entries[0]['Count']; ?>
          </span>
        </a> 
        <a href="JavaScript:showRelationList('<?php echo $_SESSION['language']; ?>','grotto','caver','<?php echo $id; ?>');" title="Voir la liste"><!--Voir la liste-->
          <span class="value_grotto">
            <?php echo $grottoes[0]['Count']; ?>
          </span>
        </a> 
        <a href="JavaScript:showRelationList('<?php echo $_SESSION['language']; ?>','contribution','caver','<?php echo $id; ?>');" title="Voir la liste"><!--Voir la liste-->
        ...
        <!--<span class="value" id="GC_IW_isConnected" style="display:none;float:right;">(est connecté)</span>--><!--Est connecté-->
    	</div>
    	<div class="label">
...
<?php } ?>
        ...
        Itinéraire<!--Itinéraire--> : <a href="JavaScript:getDirectionsForm(false);">Vers ce lieu<!--Vers ce lieu--></a> - <a href="JavaScript:getDirectionsForm(true);">Depuis ce lieu<!--Depuis ce lieu--></a>
      </div>
		</div>
		<div class="footer">
			<label for="link"><input type="checkbox" name="link" id="GC_IW_link" class="inputIW" title="Afficher les liens" style="border: none;" onclick="Javascript:switchLines(<?php echo $id; ?>, 'caver', this.checked);" />Liens<!--Liens--></label><!--Afficher les liens-->
			<span class="details">
<?php
      if ($caver[0]['Id_comment'] != "" && False) { //DESACTIVE !!!
?>
        <a href="comments_<?php echo $_SESSION['language']; ?>.php?caver_id=<?php echo $id; ?>" target="_blank" title="Voir les commentaires déposés par ce spéléologue"><!--Voir les commentaires déposés par ce spéléologue-->
				  Commentaires...<!--Commentaires...-->
			  </a><br />
<?php
      }
?>
        <a href="#" onclick="JavaScript:detailMarker(event, 'caver', '<?php echo $id; ?>', '<?php echo $_SESSION['language']; ?>');"  title="Obtenir les propriétés de ce marqueur"><!--Obtenir les propriétés de ce marqueur-->
				  Propriétés<!--Propriétés-->
			  </a><br />
			  <a href="contact_<?php echo $_SESSION['language']; ?>.php?type=message&amp;subject=bad_content&amp;category=caver&amp;name=<?php echo $caver[0]['Login']; ?>" target="filter" title="Signaler un abus"><!--Signaler du contenu hors-charte-->
				  Signaler un abus<!--Hors-charte-->
        </a>
      </span>
		</div>
<?php
...
?>
  	<div class="header cavers_clustered" style="height:42px;">
  		Spéléologues<!--Speleologues-->
  	</div>
    <div class="content" style="height:170px;">
      <div class="info">
        <?php echo getTopBubble(); ?>
        Pour détailler chaque élément, veuillez zoomer.<!--Pour détailler chaque marqueur, veuillez zoomer.-->
...
<?php while(list($k,$v) = each($cavers)) { if ($v['Nickname'] != "") { ?>
            ...
            <a href="#" onclick="JavaScript:detailMarker(event, 'caver', '<?php echo $v['Id']; ?>', '<?php echo $_SESSION['language']; ?>');" title="Obtenir les propriétés de ce marqueur"><!--Obtenir les propriétés de ce marqueur-->
    				  Propriétés<!--Propriétés-->
    			  </a>
    			</td></tr>
<?php } } ?>
