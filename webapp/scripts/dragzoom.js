...
function load_dragzoom() {
  ...
  DragZoomControl.prototype.initialize = function(map) {
    ...
    // add event listeners
    google.maps.Event.addDomListener(buttonDiv, 'click', function(e) {
      me.buttonclick_(e);
    });
    google.maps.Event.addDomListener(backButtonDiv, 'click', function(e) {
  ...
  DragZoomControl.prototype.mouseup_ = function(e){
    ...
    if (G.draggingOn) {
      ...
      if (rect.width >= G.options.minDragSize && rect.height >= G.options.minDragSize) {
        ...
        var zoomAreaPoly = new google.maps.Polyline([nw, ne, se, sw, nw], G.style.outlineColor, G.style.outlineWidth + 1,.4);
      ...
      if (G.options.stickyZoomEnabled) {
        //google.maps.Log.write("stickyZoomEnabled, re-initting");
        this.initCover_();
