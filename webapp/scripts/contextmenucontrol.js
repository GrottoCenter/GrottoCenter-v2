 ...
 * @property {Boolean} [dirsFrom = [enabled = true, label = 'Directions from here']] Shows "Directions from here" item.
 * @property {Boolean} [dirsTo = [enabled = true, label = 'Directions to here']] Shows "Directions to here" item.
 * @property {Boolean} [zoomIn = [enabled = true, label = 'Zoom in']] Shows "Zoom in" item.
 * @property {Boolean} [zoomOut = [enabled = true, label = 'Zoom out']] Shows "Zoom out" item.
 * @property {Boolean} [centerMap = [enabled = true, label = 'Center map here']] "Shows "Center map here" item.
 * @property {Boolean} [whatsHere = [enabled = true, label = 'What's here?']] "Shows "What's here?" item.
 */
 ...
 * @constructor
 */
function ContextMenuControl(language, opt_opts) {
  this.language = language;
  //if (opt_opts) {
    this.options = {
      dirsFrom: {enabled: (opt_opts ? opt_opts.dirsFrom.enabled : false),
                label: (opt_opts ? opt_opts.dirsFrom.label : 'Directions from here')},
      dirsTo: {enabled: (opt_opts ? opt_opts.dirsTo.enabled : false),
                label: (opt_opts ? opt_opts.dirsTo.label : 'Directions to here')},
      zoomIn: {enabled: (opt_opts ? opt_opts.zoomIn.enabled : false),
                label: (opt_opts ? opt_opts.zoomIn.label : 'Zoom in')},
      zoomOut: {enabled: (opt_opts ? opt_opts.zoomOut.enabled : false),
                label: (opt_opts ? opt_opts.zoomOut.label : 'Zoom out')},
      centerMap: {enabled: (opt_opts ? opt_opts.centerMap.enabled : false),
                label: (opt_opts ? opt_opts.centerMap.label : 'Center map here')},
      whatsHere: {enabled: (opt_opts ? opt_opts.whatsHere.enabled : false),
                label: (opt_opts ? opt_opts.whatsHere.label : 'What\'s here?')},
      refreshData: {enabled: (opt_opts ? opt_opts.refreshData.enabled : true),
                label: (opt_opts ? opt_opts.refreshData.label : 'Refresh')},
      copyLatLng: {enabled: (opt_opts ? opt_opts.copyLatLng.enabled : true),
                label: (opt_opts ? opt_opts.copyLatLng.label : 'Copy these coords.')},
      convertLatLng: {enabled: (opt_opts ? opt_opts.convertLatLng.enabled : true),
                label: (opt_opts ? opt_opts.convertLatLng.label : 'Convert these coords.')},
      elevationHere: {enabled: (opt_opts ? opt_opts.elevationHere.enabled : true),
                label: (opt_opts ? opt_opts.elevationHere.label : 'Elevation here')},
      properties: {enabled: (opt_opts ? opt_opts.properties.enabled : false),
                label: (opt_opts ? opt_opts.properties.label : 'Properties')},
      details: {enabled: (opt_opts ? opt_opts.details.enabled : false),
                label: (opt_opts ? opt_opts.details.label : 'Detailed sheet...')},
      editMe: {enabled: (opt_opts ? opt_opts.editMe.enabled : false),
                label: (opt_opts ? opt_opts.editMe.label : 'Edit')},
      deleteMe: {enabled: (opt_opts ? opt_opts.deleteMe.enabled : false),
                label: (opt_opts ? opt_opts.deleteMe.label : 'Delete')},
      addEntry: {enabled: (opt_opts ? opt_opts.addEntry.enabled : false),
                label: (opt_opts ? opt_opts.addEntry.label : 'Add an entry here')},
      addGrotto: {enabled: (opt_opts ? opt_opts.addGrotto.enabled : false),
                label: (opt_opts ? opt_opts.addGrotto.label : 'Add a grotto here')}
    };
  /*} else {
    this.options = {};
  }*/
...
function load_contextmenucontrol() {
  /**
   * Extends GOverlay class from the Google Maps API.
   *  Second param (selectable) should be set to true
   * @private
   */
  ContextMenuControl.prototype = new google.maps.Control(false, true);
  ...
  ContextMenuControl.prototype.initialize = function (map) {
    ...
    google.maps.Event.addListener(map, "singlerightclick", function (pixelPoint, src, ov) {
     // Right click on a dir marker
      ...
      me.options.properties.enabled = false;
      me.options.details.enabled = false;
      me.options.editMe.enabled = false;
      me.options.deleteMe.enabled = false;
      if (ov != null) {
        me.clickedPoint_ = me.rej_;
        me.markerCategory_ = ov.category;
        me.markerId_ = ov.id;
        me.markerType_ = ov.markerType;
        if (me.markerType_ === "m") {
          me.options.properties.enabled = true;
          if (me.markerCategory_ === "entry") {
            me.options.details.enabled = true;
          }
          if (me.markerCategory_ !== "caver") {
            if (userConnected === true) {
              me.options.editMe.enabled = me.options.addEntry.enabled;
              me.options.deleteMe.enabled = me.options.addGrotto.enabled;
            }
          }
        }
      } else {
        me.clickedPoint_ = map.fromContainerPixelToLatLng(pixelPoint);
        me.markerCategory_ = undefined;
        me.markerId_ = undefined;
      }
      me.rebuildMenu_();
      ...
      var pos = new google.maps.ControlPosition(G_ANCHOR_TOP_LEFT, new google.maps.Size(posX, posY));
    ...
    google.maps.Event.addDomListener(mapdiv, "mouseout", function (e) {
  ...
  ContextMenuControl.prototype.searchDirs_ = function (letter) {
    ...
    if (me.actual) {
      ...
      var marker = new google.maps.Marker(point, {icon: icon, draggable: true, title: 'Drag', bouncy: false, dragCrossMove: true});
      me.actual = marker;
  ...
  ContextMenuControl.prototype.createContextMenu_ = function (change) {
    ...
    if (me.options.dirsFrom.enabled !== false && !change) {
      me.menuList.appendChild(me.createListItem_(me.options.dirsFrom.label, "from"));//Directions from here
    }
    if (me.options.dirsTo.enabled !== false && !change) {
      me.menuList.appendChild(me.createListItem_(me.options.dirsTo.label, "to"));//Directions to here
    }
    if (change == "add") {
      me.menuList.appendChild(me.createListItem_("Add a destination", "add"));//Add a destination
    }
    if (change == "remove") {
      me.menuList.appendChild(me.createListItem_("Remove this point", "rem"));//Remove this point
    }
    if (me.options.zoomIn.enabled !== false) {
      me.menuList.appendChild(me.createRuler_());
      me.menuList.appendChild(me.createListItem_(me.options.zoomIn.label, "in"));//Zoom In
    }
    if (me.options.zoomOut.enabled !== false) {
      me.menuList.appendChild(me.createListItem_(me.options.zoomOut.label, "out"));//Zoom Out
    }
    if (me.options.centerMap.enabled !== false) {
      me.menuList.appendChild(me.createListItem_(me.options.centerMap.label, "center"));//Center Map here
      me.menuList.appendChild(me.createRuler_());
    }
    if (me.options.refreshData.enabled !== false) {
      me.menuList.appendChild(me.createListItem_(me.options.refreshData.label, "refresh"));//Refresh
      me.menuList.appendChild(me.createRuler_());
    }
    if (me.options.addEntry.enabled !== false) {
      me.menuList.appendChild(me.createListItem_(me.options.addEntry.label, "addentry"));//Add an entry here
    }
    if (me.options.addGrotto.enabled !== false) {
      me.menuList.appendChild(me.createListItem_(me.options.addGrotto.label, "addgrotto"));//Add a grotto here
      me.menuList.appendChild(me.createRuler_());
    }
    if (me.options.copyLatLng.enabled !== false) {
      me.menuList.appendChild(me.createListItem_(me.options.copyLatLng.label, "copy"));//Copy these coords.
    }
    if (me.options.convertLatLng.enabled !== false) {
      me.menuList.appendChild(me.createListItem_(me.options.convertLatLng.label, "convert"));//Convert these coords.
    }
    if (me.options.elevationHere.enabled !== false) {
      me.menuList.appendChild(me.createListItem_(me.options.elevationHere.label, "elevation"));//Elevation here
    }
    if (me.options.properties.enabled !== false) {
      me.menuList.appendChild(me.createRuler_());
      me.menuList.appendChild(me.createListItem_(me.options.properties.label, "properties"));//Properties
    }
    if (me.options.details.enabled !== false) {
      me.menuList.appendChild(me.createListItem_(me.options.details.label, "details"));//Detailed sheet...
    }
    if (me.options.editMe.enabled !== false) {
      me.menuList.appendChild(me.createListItem_(me.options.editMe.label, "edit"));//Edit
    }
    if (me.options.deleteMe.enabled !== false) {
      me.menuList.appendChild(me.createListItem_(me.options.deleteMe.label, "delete"));//Delete
    }
    if (me.options.whatsHere.enabled !== false) {
      me.menuList.appendChild(me.createRuler_());
      me.menuList.appendChild(me.createListItem_(me.options.whatsHere.label, "here"));//What\'s here?
    }
    me.hideMenu_();
    // Adds context menu to the map container
    me.map_.getContainer().appendChild(me.menuList);
    pos = new google.maps.ControlPosition(G_ANCHOR_TOP_LEFT, new google.maps.Size(0, 0));
    pos.apply(me.menuList);
    return me.menuList;
  };
  ...
  ContextMenuControl.prototype.createListItem_ = function (text, arg) {
    var me = this;
		...
		aNode.setAttribute('class', 'nothing');
    ...
    google.maps.Event.addDomListener(entry, "mouseover", function () {
      ...
      entry.style.backgroundColor = "#d1fAff";
    });
    ...
    google.maps.Event.addDomListener(entry, "click", function (event) {
      var point;
      point = me.clickedPoint_;
      if (arg == "from") {
        me.searchDirs_("A");
      } else if (arg == "to") {
        me.searchDirs_("B");
        ...
        copySelectedCoords({'lat': point.lat(), 'lng': point.lng()});
      } else if (arg == "convert") {
        convertMousePsn({'lat': point.lat(), 'lng': point.lng()});
      } else if (arg == "elevation") {
        showMouseElevation({'lat': point.lat(), 'lng': point.lng()});
      } else if (arg == "properties") {
        detailMarker(event, me.markerCategory_, me.markerId_, me.language);
      } else if (arg == "details") {
        detailMarker(event, me.markerCategory_, me.markerId_, me.language, true);
      } else if (arg == "edit") {
        editMarker(me.markerCategory_, me.markerId_, me.language);
      } else if (arg == "delete") {
        deleteMarker(me.markerCategory_, me.markerId_, me.language);
      } else if (arg == "addgrotto") {
        addMarker('grotto',undefined,undefined,me.language);
      } else if (arg == "addentry") {
        addMarker('entry',undefined,undefined,me.language);
      }
  ...
  ContextMenuControl.prototype.createIcon_ = function (image) {
    ...
    if (image == "arrow") {
      ...
      icon.iconAnchor = new google.maps.Point(20, 34);
      ...
      icon.infoWindowAnchor = new google.maps.Point(19, 2);
    }
    return icon;
  };
  ...
  ContextMenuControl.prototype.makeDirMarker_ = function (point, letter) {
    ...
    var icon = me.createIcon_(image);
    var marker = new google.maps.Marker(point, {icon: icon, draggable: true, title: 'Drag to change route', bouncy: false, dragCrossMove: true});
  ...
  ContextMenuControl.prototype.getDirs_ = function (points, addr) {
    var me = this;
    me.gdir = new google.maps.Directions();
    google.maps.Event.bind(me.gdir, "error", me, me.handleErrors_);
    google.maps.Event.bind(me.gdir, "load", me, me.dirsLoad_);
  ...
  ContextMenuControl.prototype.makeForm_ = function (letter) {
    ...
    google.maps.Event.addDomListener(rlink, "click", function () {
      if (me.actual) {
        me.map_.removeOverlay(me.actual);
        me.actual = null;
      }
  ...
  ContextMenuControl.prototype.getReverseGeocode_ = function () {
    ...
    var arrow_mark = new google.maps.Marker(point, {icon: icon, title: p_string });
    me.heremarks_.push(arrow_mark);
    me.map_.addOverlay(arrow_mark);
  
    google.maps.Event.addListener(arrow_mark, "click", function () {
    ...
    geo.getLocations(point, function (response) {
      if (response.Status.code == 200) {
        ...
        var red_mark = new google.maps.Marker(latlng);
        ...
        google.maps.Event.addListener(red_mark, "click", function () {
