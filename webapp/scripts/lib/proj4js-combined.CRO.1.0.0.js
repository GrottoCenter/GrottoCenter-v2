 ...
 *
     ...
     */
    scriptName: "proj4js-combined.CRO.1.0.0.js",
    ...
    loadScript: function(url, onload, onfail, loadCheck) {
      Proj4js.reportError('[!] [loadScript] loading '+url);
			//CRO 2010-09-06: modifications / adaptations
      var script, head;
			script = $('<script />', {src:url, id:url, type:'text/javascript', defer:false})[0];
			script.loadCheck = loadCheck; //IE workaround since there is no onerror handler
			head = $('head')[0] || document.documentElement;
      script.onload = function() {
				Proj4js.reportError('[+] [loadScript] success, url: '+url);
				if (loadCheck()) {
					Proj4js.reportError('[+] [loadScript] check success, url: '+url);
					onload();
				} else {
					Proj4js.reportError('[-] [loadScript] check error, url: '+url);
					onfail();
				}
			};
      script.onerror = function() {
				Proj4js.reportError('[-] [loadScript] error, url: '+url);
				onfail();
			};
      if (/MSIE/.test(navigator.userAgent)) { //IE workaround since there is no onerror handler
    ...
    checkReadyState: function() {
			var loadCheck;
      if ($(this).attr('readyState') == 'loaded') {
        if (!this.loadCheck()) {
					Proj4js.reportError('[-] [checkReadyState] check error');
          this.onerror();
        } else {
					Proj4js.reportError('[+] [checkReadyState] check success');
          this.onload();
        }
      }
    }
...
Proj4js.Proj = Proj4js.Class({
  ...
  */
  //CRO 2010-09-04 adding callback function
  initialize: function(srsCode, callback) {
      this.srsCodeInput = srsCode;
      //CRO 2010-09-04 adding callback function
      this.callBackFunc = callback;
      // DGR 2008-08-03 : support urn and url
      if (srsCode.toUpperCase().indexOf('URN:') == 0) {
          //urn:ORIGINATOR:def:crs:CODESPACE:VERSION:ID //urn:ogc:def:crs:epsg:VERSION:2326
          var urn = srsCode.split(':');
          if ((urn[1] == 'ogc' || urn[1] =='x-ogc') &&
              (urn[2] =='def') &&
              (urn[3] =='crs')) {
              srsCode = urn[4]+':'+urn[urn.length-1]; //CODESPACE:ID
          }
      } else if (srsCode.toUpperCase().indexOf('HTTP://') == 0) {
          //url#ID
      ...
      if (this.srsCode.indexOf("EPSG:") == 0) {
          ...
          this.srsProjNumber = this.srsCode.substring(5);
			// CRO 2010-09-08
      } else if (this.srsCode.indexOf("ESRI:") == 0) {
          this.srsCode = this.srsCode;
          this.srsAuth = 'esri';
          this.srsProjNumber = this.srsCode.substring(5);
      } else if (this.srsCode.indexOf("IAU2000:") == 0) {
          this.srsCode = this.srsCode;
          this.srsAuth = 'iau2000';
          this.srsProjNumber = this.srsCode.substring(8);
      } else if (this.srsCode.indexOf("SR-ORG:") == 0) {
          this.srsCode = this.srsCode;
          this.srsAuth = 'sr-org';
          this.srsProjNumber = this.srsCode.substring(7);
...
//CRO 2010-06-02 : accessor to the lookup service
    ...
    defsLoaded: function() {
      this.parseDefs();
			...
			if (this.projName == null) {
				Proj4js.reportError('[-] projName is null!');
				this.defsFailed();
			} else {
...
Proj4js.defs = {
  ...
  'WGS84': "+title=GPS +proj=longlat +ellps=WGS84 +datum=WGS84 +units=degrees"
	/*,
	'EPSG:27561': "+title=Lamber I +proj=lcc +lat_1=49.50000000000001 +lat_0=49.50000000000001 +lon_0=0 +k_0=0.999877341 +x_0=600000 +y_0=200000 +a=6378249.2 +b=6356515 +towgs84=-168,-60,320,0,0,0,0 +pm=paris +units=m +no_defs",
...
/*
Proj4js.defs['Lambert I'] = Proj4js.defs['EPSG:27561'];
...
/* ======================================================================
    defs/GOOGLE.js (don't undestand that...)
   ====================================================================== */
